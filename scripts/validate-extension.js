...
expect(returnCoordinatorSource.includes("const OPEN_SEI_PROCESS_MESSAGE = 'sei-protocolistas:open-sei-process'"), 'FAST PROC: mensagem para reutilizar aba do SEI não encontrada')
expect(returnCoordinatorSource.includes("callApi(api.tabs, 'query', { url: SEI_MATCH_PATTERN })"), 'FAST PROC: busca por aba existente do SEI não encontrada')
expect(returnCoordinatorSource.includes('async function openOrReuseSei'), 'FAST PROC: coordenador de reutilização da aba do SEI não encontrado')
expect(fastMailSource.includes('await runtimeMessage({ type: OPEN_SEI_PROCESS_MESSAGE })'), 'FAST MAIL: abertura de processo deve usar o coordenador de abas')
expect(!fastMailSource.includes("window.open('about:blank', '_blank')"), 'FAST MAIL: abertura de processo não deve criar aba diretamente')
...
expect(fastMailSource.includes('OPEN_SEI_PROCESS_MESSAGE'), 'FAST MAIL: deve solicitar ao coordenador a abertura do SEI')
