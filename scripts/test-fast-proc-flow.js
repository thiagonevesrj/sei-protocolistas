...
function testIsolatedQuickAuthentication () {
  ...
  assert.ok(source.includes('const nativeTrigger = nativeAuthenticationTrigger()'))
  assert.ok(source.includes('? { quickRequest, nativeTrigger }'))
  assert.ok(source.includes('AUTHENTICATION_AVAILABILITY_WAIT'))
  ...
  assert.ok(source.includes('function isAuthenticationCommand (element)'))
  ...
  assert.ok(source.includes('image.closest(clickableSelector) || image'))
