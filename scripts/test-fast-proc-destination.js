...
assert(handoff.includes("inputType: 'insertText'"), 'FAST PROC destino: pesquisa deve simular a digitação esperada pelo campo legado do SEI')
assert(handoff.includes("['keydown', 'keyup']"), 'FAST PROC destino: pesquisa deve disparar eventos completos de teclado')
assert(handoff.includes('Date.now() - lastSearchAt >= 1200'), 'FAST PROC destino: pesquisas repetidas devem aguardar a resposta do SEI')
assert(handoff.includes('function pressKey (element, key, code, keyCode)'), 'FAST PROC destino: deve simular as teclas do componente legado')
assert(handoff.includes("pressKey(element, 'ArrowDown', 'ArrowDown', 40)"), 'FAST PROC destino: deve pré-selecionar a sugestão com seta para baixo')
assert(handoff.includes("pressKey(element, 'Enter', 'Enter', 13)"), 'FAST PROC destino: deve confirmar a unidade com Enter')
assert(handoff.includes('pressArrowDown(input)\n        pressEnter(input)'), 'FAST PROC destino: deve repetir a sequência manual seta para baixo e Enter')
...
assert(handoff.includes('$' + '{destination} ENCONTRADO. PRÉ-SELECIONANDO E CONFIRMANDO…'), 'FAST PROC destino: deve orientar durante a confirmação com seta e Enter')
...
assert(handoff.includes('}, 450)'), 'FAST PROC destino: aviso verde deve aguardar a confirmação do componente legado')
